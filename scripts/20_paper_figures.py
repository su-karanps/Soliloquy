...
for ax, (label, p) in zip(axes1, model_probe_data):
    ...
    for pos, color in pos_colors.items():
        if pos in curves:
            c = curves[pos]
            ax.plot(range(len(c)), c, color=color, linewidth=1.8,
                    label=pos.replace("_", " "))
    bp = p.get("best_probe", {})
    if bp:
        ax.axvline(bp["layer"], color=GRAY, linewidth=0.8, linestyle="-.")
        ax.text(bp["layer"] + 0.3, 0.47, f"best L{bp['layer']}\nAUC={bp['auc']:.3f}",
...
fig1.suptitle("Fig 1 — Correctness probe AUC by layer (SimpleQA, forced prompt)", fontsize=11)
fig1.tight_layout()
fig1.savefig(OUT / "fig1_probe_auc_by_layer.png", dpi=160, bbox_inches="tight")
plt.close(fig1)
...
for ax, (label, pt) in zip(axes2, model_patch_data):
    ...
    ls = range(len(rescue))
    ax.plot(ls, rescue, color=GREEN, linewidth=2.0, label="rescue")
    ax.fill_between(ls, 0, rescue, where=(np.array(rescue) > 0), alpha=0.15, color=GREEN)
    if corruption:
        ax.plot(ls, corruption, color=RED, linewidth=1.4, linestyle="--", alpha=0.8,
                label="corruption")
    ax.axhline(0, color=GRAY, linewidth=0.6, linestyle=":")
    peak = pt.get("peak_rescue_layer")
    if peak is not None:
        ax.axvline(peak, color=GREEN, linewidth=0.8, linestyle="-.", alpha=0.5)
        ax.text(peak + 0.3, max(rescue) * 0.88, f"L{peak}", color=GREEN, fontsize=9)
    ax.set_xlabel("Layer")
    ax.set_title(label, fontsize=10)
    spine_clean(ax)
axes2[0].set_ylabel("Δ logit-diff (rescue effect)")
axes2[0].legend(frameon=False, fontsize=8)
fig2.suptitle("Fig 2 — Residual-stream patching rescue effect by layer (SimpleQA)", fontsize=11)
fig2.tight_layout()
fig2.savefig(OUT / "fig2_rescue_by_layer.png", dpi=160, bbox_inches="tight")
plt.close(fig2)
...
for ax, (label, p, pt) in zip(axes3, model_all_data):
    ...
    if not rescue_m:
        ax.text(0.5, 0.5, "No patch data", transform=ax.transAxes, ha="center")
        ax.set_title(label); continue
    ax2 = ax.twinx()
    # Probe curves (left axis, dashed)
    for pos, color in pos_colors.items():
        if pos in curves_m:
            c = curves_m[pos]
            ax.plot(range(len(c)), c, color=color, linewidth=1.2, alpha=0.65,
                    linestyle="--")
    ax.set_ylim(0.45, 1.0)
    ax.axhline(0.5, color=GRAY, linewidth=0.5, linestyle=":")
    ax.set_ylabel("Probe AUC", color="#555", fontsize=8)
    ax.tick_params(axis="y", colors="#555", labelsize=8)
    # Rescue (right axis, solid)
    ax2.plot(range(len(rescue_m)), rescue_m, color=GREEN, linewidth=2.2, label="rescue")
    ax2.fill_between(range(len(rescue_m)), 0, rescue_m,
                     where=(np.array(rescue_m) > 0), alpha=0.15, color=GREEN)
    ax2.axhline(0, color=GRAY, linewidth=0.5, linestyle=":")
    ax2.set_ylabel("Δ logit-diff", color=GREEN, fontsize=8)
    ...
    if best_L is not None:
        ax.axvline(best_L, color="#bbb", linewidth=0.8, linestyle="-.")
        ax.text(best_L + 0.2, 0.47, f"L{best_L}", color="#888", fontsize=7.5)
    if peak_L is not None:
        ax2.axvline(peak_L, color=GREEN, linewidth=0.8, linestyle="-.")
        ax2.text(peak_L + 0.2, max(rescue_m) * 0.86, f"L{peak_L}", color=GREEN, fontsize=7.5)
    gap = (peak_L - best_L) if (peak_L is not None and best_L is not None) else "?"
    ax.set_xlabel("Layer", fontsize=8)
    ax.set_title(f"{label}\ngap = {gap:+d} layers" if isinstance(gap, int) else label,
                 fontsize=9)
    ax.spines["top"].set_visible(False)
    ax2.spines["top"].set_visible(False)
    ax.grid(alpha=0.15)
# Shared legend for probe positions (dashed lines)
from matplotlib.lines import Line2D
legend_els = [Line2D([0], [0], color=c, linestyle="--", linewidth=1.4,
                     label=pos.replace("_", " "))
              for pos, c in pos_colors.items()]
legend_els.append(Line2D([0], [0], color=GREEN, linewidth=2, label="rescue effect"))
axes3[0].legend(handles=legend_els, fontsize=7.5, frameon=False, loc="upper left")
fig3.suptitle("Fig 3 — Decodability (probe AUC) vs causality (rescue effect) by layer",
              fontsize=11)
fig3.tight_layout()
fig3.savefig(OUT / "fig3_decodability_vs_causality.png", dpi=160, bbox_inches="tight")
plt.close(fig3)
...
axes[0].set_xticks(x)
axes[0].set_xticklabels([f"{m}\n(peak L{l})" for m, l in zip(models, peak_layers)], fontsize=8)
axes[0].set_ylabel("Rescue effect (Δ logit-diff)")
axes[0].set_title("Fig 4a — MLP vs attention rescue at peak layer")
...
axes[1].set_xticklabels(datasets_t, fontsize=8, rotation=15, ha="right")
axes[1].set_ylabel("Rescue effect (Δ logit-diff)")
axes[1].set_title("Fig 4b — Dataset transfer: MLP vs attn (Qwen-3B)")
axes[1].legend(frameon=False, fontsize=9)
spine_clean(axes[1])

fig4.tight_layout()
fig4.savefig(OUT / "fig4_component_localization.png", dpi=160, bbox_inches="tight")
plt.close(fig4)
...
if steer3_rows:
    ...
    axes5[0].plot(alphas, correct_rates, color=BLUE, marker="o", linewidth=1.8,
                  label="correct rate")
    axes5[0].axhline(baseline, color=GRAY, linewidth=0.8, linestyle="--",
                     label=f"baseline = {baseline:.3f}")
    axes5[0].set_xlabel("Steering α")
    axes5[0].set_ylabel("Correct rate")
    axes5[0].set_title("Fig 5a — Prompt-level steering along probe dir\n(Qwen-3B, L15 answer_first)")
    axes5[0].legend(frameon=False, fontsize=9)
    axes5[0].set_ylim(-0.01, max(correct_rates) * 1.4 + 0.01)
    spine_clean(axes5[0])
else:
    axes5[0].text(0.5, 0.5, "No steering data", transform=axes5[0].transAxes, ha="center")
    axes5[0].set_title("Fig 5a — Prompt-level probe steering")
    ...
    for mode, color in mode_colors.items():
        ...
        if rs:
            axes5[1].plot([r["alpha"] for r in rs], [r["correct_rate"] for r in rs],
                          color=color, marker="o", linewidth=1.6, label=mode)
    axes5[1].set_xlabel("Steering α")
    axes5[1].set_ylabel("Correct rate")
    axes5[1].set_title("Fig 5b — Late-MLP directional steering\n(Qwen-3B, L35 MLP output)")
    axes5[1].legend(frameon=False, fontsize=9)
    spine_clean(axes5[1])
else:
    axes5[1].text(0.5, 0.5, "No MLP steering data", transform=axes5[1].transAxes, ha="center")
    axes5[1].set_title("Fig 5b — Late-MLP directional steering")

fig5.suptitle("Fig 5 — Steering negative result: 1-D probe direction does not control generation",
              fontsize=10)
fig5.tight_layout()
fig5.savefig(OUT / "fig5_steering_negative.png", dpi=160, bbox_inches="tight")
plt.close(fig5)
...
categories_q = ["correct\n+confident", "correct\n+unconfident",
                "incorrect\n+confident", "incorrect\n+unconfident"]
bar_colors_q = [GREEN, "#aec7e8", RED, ORANGE]

for ax6, (label, d) in zip(axes6, model_dissoc):
    if not d:
        ax6.text(0.5, 0.5, "No dissoc data", transform=ax6.transAxes, ha="center")
    ...
    ax6.set_xticklabels(categories_q, fontsize=7.5)
    ax6.set_title(f"{label}\nAUC={auc:.3f}  cos(corr,conf)={cos:.3f}", fontsize=9)
    ax6.set_ylim(0, max(pcts) * 1.25 if pcts else 100)
    spine_clean(ax6)

axes6[0].set_ylabel("% of responses")
fig6.suptitle("Fig 6 — Correctness vs verbal-confidence dissociation (SimpleQA, forced prompt)",
              fontsize=10)
fig6.tight_layout()
fig6.savefig(OUT / "fig6_confidence_dissociation.png", dpi=160, bbox_inches="tight")
plt.close(fig6)

# ─── Multi-model comparison (bonus) ──────────────────────────────────────────
fig_mm, axes_mm = plt.subplots(1, 3, figsize=(12, 4))
model_names = ["Qwen-3B\n(36L)", "Qwen-7B\n(28L)", "Llama-3.1-8B\n(32L)"]
...
for i, (v, l) in enumerate(zip(probe_aucs, probe_layers)):
    axes_mm[0].text(i, v + 0.005, f"{v:.3f}\n(L{l})", ha="center", fontsize=8)
axes_mm[0].axhline(0.5, color=GRAY, linewidth=0.7, linestyle=":")
axes_mm[0].set_xticks(x_mm); axes_mm[0].set_xticklabels(model_names, fontsize=8)
...
axes_mm[1].set_xticks(x_mm); axes_mm[1].set_xticklabels(model_names, fontsize=8)
...
axes_mm[2].set_xticks(x_mm); axes_mm[2].set_xticklabels(model_names, fontsize=8)
axes_mm[2].set_ylabel("Component rescue effect"); axes_mm[2].set_title("MLP vs attention at peak layer")
axes_mm[2].legend(frameon=False, fontsize=9); spine_clean(axes_mm[2])

fig_mm.suptitle("Multi-model comparison — Qwen-3B / Qwen-7B / Llama-3.1-8B (SimpleQA)",
                fontsize=10)
fig_mm.tight_layout()
fig_mm.savefig(OUT / "fig_multimodel.png", dpi=160, bbox_inches="tight")
plt.close(fig_mm)
